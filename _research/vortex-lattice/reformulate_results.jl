...
span_wing     = [8]
chord_wing    = 6
wing_vlm_mesh = chord_coordinates(wing, span_wing, chord_wing)
wing_cam_mesh = camber_coordinates(wing, span_wing, chord_wing)
...
span_htail     = [6]
chord_htail    = 6
htail_vlm_mesh = chord_coordinates(htail, span_htail, chord_htail)
htail_cam_mesh = camber_coordinates(htail, span_htail, chord_htail)
...
# Aircraft
Γs = circulations(data[ac_name])
horsies = 

# Wing
wing_CFs = surface_force_coefficients(data["Wing"])
wing_Γs  = circulations(data["Wing"])

# Horizontal tail
htail_CFs = surface_force_coefficients(data["Horizontal Tail"])
htail_Γs  = circulations(data["Horizontal Tail"])

# Vertical tail
vtail_Γs  = circulations(data["Vertical Tail"])


# Γs = data[ac_name][end]
# CFs_wing, CMs_wing, wing_horsies, Γ0_wing = data["Wing"][3:end];
# CFs_htail, CMs_htail, hs_htail, Γ0_htail = data["Horizontal Tail"][3:end];
# Γ0_vtail = data["Vertical Tail"][end];

## Wing FEM setup
wing_vlm_acs    = bound_leg_center.(wing_horsies)
wing_vlm_forces = force.(wing_CFs, dynamic_pressure(ρ, V), S)

wing_fem_weight = 0.40
wing_fem_mesh   = make_beam_mesh(wing_vlm_mesh, wing_fem_weight)
