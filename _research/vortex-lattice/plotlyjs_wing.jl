...
trace_panels(panels :: Vector{<: Panel3D}) = trace_coords(panel_splits(panel_coordinates.(panels))...)

trace_surface(wing :: AbstractWing, span_num = 5, chord_num = 30; rotation = [1 0 0; 0 1 0; 0 0 1], translation = SVector(0, 0, 0)) = trace_panels([ (translation ∘ rotation)(panel) for panel in mesh_wing(wing, span_num, chord_num)[:] ])

function trace_panels(panels :: Vector{<: Panel3D}, Γs :: Vector{<: Real})
    coords = panel_coordinates.(panels[:])
