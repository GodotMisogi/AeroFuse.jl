...
"""
arc_length(foil::Foil) =
    let c = coordinates(foil)
        @views norm(c[2:end, :] .- c[1:end-1, :])
    end


chord_length(foil::Foil) = maximum(foil.x) - minimum(foil.x)
...
@views function interpolate(foil::Foil, xs)
    ...
    return Foil([xs[end:-1:2]; xs], [y_u[end:-1:2]; y_l], foil.name)
end
...
function cosine_interpolation(foil, n::Integer = 40)
    ...
    return interpolate(foil, x_circ)
end
...
"""
make_panels(foil::Foil) =
    @views Panel2D.(foil.x[2:end], foil.y[2:end], foil.x[1:end-1], foil.y[1:end-1])[end:-1:1]
...
camber_thickness_to_coordinates(coords) =
    @views camber_thickness_to_coordinates(coords[:, 1], coords[:, 2], coords[:, 3])
