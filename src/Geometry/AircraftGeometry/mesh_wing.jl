...
"""
function camber_coords(wing :: HalfWing, span_num :: Vector{<: Integer}, chord_num :: Integer, flip = false; spacings = "cosine")
    leading_xyz  = leading_edge(wing, flip)
    ...
    coords_chopper(foil_coords, span_num, spacings, ifelse(flip, -1, 1))
end
...
"""
mesh_horseshoes(obj :: HalfWing, span_num :: Vector{<: Integer}, chord_num :: Integer; spacings = "cosine", direction = 1, flip = false) = make_panels(wing_chopper(wing_bounds(obj, flip)..., span_num, chord_num, span_spacing = spacings, direction = direction))
...
"""
mesh_cambers(wing :: HalfWing, span_num :: Vector{<: Integer}, chord_num :: Integer; spacings = "cosine", flip = false) = make_panels(camber_coords(wing, span_num, chord_num, flip; spacings = spacings))
...
function mesh_horseshoes(wing :: Wing, span_num, chord_num, spacings = "sine")
    if wing.left === wing.right
        ...
        left_panels  = mesh_horseshoes(wing.left, span_num[end:-1:1], chord_num, spacings = spacings, direction = -1, flip = true)
        right_panels = mesh_horseshoes(wing.right, span_num, chord_num, spacings = spacings)
    else
...
function mesh_cambers(wing :: Wing, span_num, chord_num, spacings = "sine")
    if wing.left === wing.right
        # Need to consider sine distribution at symmetry, and cosine thereafter
        left_panels  = mesh_cambers(wing.left, span_num[end:-1:1], chord_num, spacings = spacings, flip = true)
        right_panels = mesh_cambers(wing.right, span_num, chord_num, spacings = spacings)
    else
...
vlmesh_wing(wing :: Union{Wing, HalfWing}, span_num, chord_num, spacings = "sine") = mesh_horseshoes(wing, span_num, chord_num, spacings), mesh_cambers(wing, span_num, chord_num, spacings)
