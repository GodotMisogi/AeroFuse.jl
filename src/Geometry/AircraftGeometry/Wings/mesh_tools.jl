...
chop_coordinates(coords, n::Integer, space::AbstractSpacing) =
    @views vcat(mapreduce((c1, c2) -> chop_sections(c1, c2, n, space), vcat, coords[1:end-1], coords[2:end]), [coords[end]])
...
function transform_coordinates(xyz, rot, section)
    res = (xyz * rot) .+ permutedims(section)
    return [SVector{3}(row) for row in eachrow(res)]
end

@views function chop_spanwise_sections(scaled_foils, twisties, leading_xyz, span_nums, spacings, symmetry=false, flip=false)

    if symmetry
        scaled_foils = vcat(scaled_foils[end:-1:2], scaled_foils)
        twisties = vcat(twisties[end:-1:2], twisties)
    elseif flip
    ...
    # foil_coords = reduce(hcat, foil_coords)
