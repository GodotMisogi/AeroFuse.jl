...
function Wing(span, dihedral, sweep, sweep_ratio, taper, root_chord, root_twist, tip_twist, root_control, tip_control, root_foil, tip_foil, affine, symmetry, flip)
...
function WingSection(;
    ...
    return Wing(b_w, dihedral, sweep, sweep_ratio, taper, c_root_w, root_twist, tip_twist, root_control, tip_control, root_foil, tip_foil, affine, symmetry, flip)
end

## Standard stabilizers (non-exhaustive)
#==========================================================================================#

# HorizontalTail(; root_chord = 1.0, taper = 1.0, span = 1.0, sweep = 0., sweep_ratio = 0., root_twist = 0., tip_twist = 0., root_foil = naca4(0,0,1,2), tip_foil = root_foil, angle = 0.) = WingSection(root_chord = root_chord, taper = taper, span = span, sweep = sweep, sweep_ratio = sweep_ratio, root_twist = root_twist, tip_twist = tip_twist, root_foil = naca4(0,0,1,2), tip_foil = tip_foil, angle = angle, axis = [0., 1., 0.])

# VerticalTail(; root_chord = 1.0, taper = 1.0, span = 1.0, sweep = 0., sweep_ratio = 0., root_foil = naca4(0,0,1,2), tip_foil = root_foil) = HalfWingSection(root_chord = root_chord, taper = taper, span = span, sweep = sweep, sweep_ratio = sweep_ratio, tip_foil = tip_foil, angle = 90., axis = [1., 0., 0.])