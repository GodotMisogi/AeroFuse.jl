...
form_factor_wing(x_c, t_c, Λ_m, M) = (1 + 0.6t_c / x_c + 100t_c^4) * (1.34M^0.18 * cosd(Λ_m)^0.28)
...
function wetted_area_drag(wing :: AbstractWing, x_tr, V, ρ, a_ref, μ)
    # Chord processing
    cs = chords(wing)
    mean_chords = @views (cs[1:end-1] + cs[2:end]) / 2
...
Estimate the profile drag coefficient of a ``Wing`` using the **wetted-area method** based on Schlichting's skin-friction coefficient formula with specified transition ratio(s) ``xₜᵣ`` and given `References`.
"""
...
Estimate the profile drag coefficient of a `Wing` using the **local-friction and local-dissipation method** based on Schlichting's skin-friction coefficient formula with specified transition ratio(s) ``xₜᵣ`` and given `References`.
"""
