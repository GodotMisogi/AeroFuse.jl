...
panel_velocities(hs_comp, Γs, horseshoes, U, Ω) = midpoint_velocity.(bound_leg_center.(hs_comp), Ref(horseshoes), Ref(Γs), Ref(U), Ref(Ω))
...
"""
nearfield_forces(Γs, horseshoes, U, Ω, ρ) = kutta_joukowsky.(Ref(ρ), Γs, panel_velocities(horseshoes, Γs, horseshoes, U, Ω), bound_leg_vector.(horseshoes))
