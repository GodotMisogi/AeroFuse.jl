...
"""
influence_coefficient(horseshoe :: Horseshoe, horseshoe_j, V_hat, finite_core = false) = @timeit "Influence Coefficient" dot(velocity(horseshoe_point(horseshoe_j), horseshoe, 1., V_hat, finite_core), horseshoe_normal(horseshoe_j))
...
"""
influence_matrix(horseshoes, V_hat, finite_core = false) = @timeit "Influence Matrix" [ influence_coefficient(horsie_j, horsie_i, V_hat, finite_core) for horsie_i ∈ horseshoes, horsie_j ∈ horseshoes ]
...
"""
boundary_condition(velocities, normals) = @timeit "Boundary Vector" dot.(velocities, normals)
