...
function solve_coupled_residual!(R, x, speed, β, ρ, Ω, vlm_mesh, cam_mesh, fem_mesh, stiffness_matrix, weight, load_factor)
    ...
    @timeit "Surface Forces" vlm_forces = surface_forces(Γ, new_horsies, U, Ω, ρ)
...
#     @timeit "Combine Horseshoes" all_horsies = [ new_horsies[:]; other_horsies ]
...
#     # Compute lift
#     D, Y, L = body_to_wind_axes(sum(mapreduce(vec, vcat, new_forces)), α, β)
