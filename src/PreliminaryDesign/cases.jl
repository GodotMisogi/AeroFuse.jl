...
"""
function solve_case(horseshoe_panels :: Matrix{<: Panel3D}, normals, freestream :: Freestream, r_ref = [0.25, 0., 0.], ρ = 1.225; symmetry = false)
	# Solve system
...
"""
	solve_case(wing :: Union{Wing, HalfWing}, freestream :: Freestream, ρ :: Real, r_ref = [0.25, 0., 0.]; span_num = 5 :: Integer, chord_num = 10 :: Integer)
...
function solve_case(wing :: Union{Wing, HalfWing}, freestream :: Freestream, ρ :: Real, r_ref = [0.25, 0., 0.]; span_num :: Integer = 5, chord_num :: Integer = 10)
	...
	panel_forces, panel_moments, wind_rates, trefftz_force, trefftz_moment, Γs, horseshoes = solve_case(horseshoe_panels, normals, freestream, r_ref, ρ)
...
"""
function solve_case(foil :: Foil, freestream :: Uniform2D; sources = false, wake_length = 1e3, num_panels :: Integer = 60)
	panels = paneller(foil, num_panels)
	cps, cls, cl_wake = solve_problem(panels, velocity(freestream), sources, wake_length)
...
#     @timeit "Pressure Distribution" cps = pressure_coefficient.(geom_forces, ρ, freestream.V, panel_area.(camber_panels[:]))
...
#     @timeit "Nearfield Coefficients" nearfield_coeffs = aerodynamic_coefficients(force, moment, drag, trans_rates, freestream.V, projected_area(wing), span(wing), mean_aerodynamic_chord(wing), ρ)

#     @timeit "Farfield Coefficients" farfield_coeffs = aerodynamic_coefficients(trefftz_force, trefftz_moment, trans_rates, freestream.V, projected_area(wing), span(wing), mean_aerodynamic_chord(wing), ρ)
