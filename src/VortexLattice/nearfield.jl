...
"""
midpoint_velocity(r :: SVector{3, Float64}, Ω :: SVector{3, Float64}, horseshoes :: Array{Horseshoe}, Γs :: Array{<: Real}, U) = sum(trailing_velocity(r, horseshoe, Γ, -normalize(U)) for (horseshoe, Γ) ∈ zip(horseshoes, Γs)) .- U .- cross(Ω, r)
...
function nearfield_forces(Γs :: Array{<: Real}, horseshoes :: Array{Horseshoe}, U, Ω, ρ)
    ...
    @timeit "Summing Forces" [ 
      ...
      ρ * Γ_i * cross(midpoint_velocity(r_i, Ω, horseshoes, Γs, U), l_i) end 
...
"""
function nearfield_dynamics(Γs :: Array{<: Real}, horseshoes :: Array{Horseshoe}, freestream :: Freestream, r_ref, ρ = 1.225)
