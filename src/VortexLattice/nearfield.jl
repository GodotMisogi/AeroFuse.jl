...
Placeholder.
"""
trailing_velocity(r, horseshoe :: Horseshoe, Γ :: Real, V) = trailing_legs_velocities(r1(r, horseshoe), r2(r, horseshoe), Γ, V)
...
"""
midpoint_velocity(r, Ω, horseshoes :: AbstractVector{<: Horseshoe}, Γs, U) = @timeit "Midpoint Velocity" sum(x -> trailing_velocity(r, x[1], x[2], U), zip(horseshoes, Γs)) - U - Ω × r
...
"""
function nearfield_forces(Γs, horseshoes :: AbstractVector{<: Horseshoe}, U, Ω, ρ :: Real)
    ρ_ref, Ω_ref, hs_ref, Γ_ref, U_ref = Ref(ρ), Ref(Ω), Ref(horseshoes), Ref(Γs), Ref(U)
    @timeit "Evaluating Forces" @. kutta_joukowsky(ρ_ref, Γs, midpoint_velocity(bound_leg_center(horseshoes), Ω_ref, hs_ref, Γ_ref, U_ref), bound_leg_vector(horseshoes))
end

sym_nearfield_forces(Γs, horseshoes :: AbstractVector{<: Horseshoe}, U, Ω, ρ :: Real) = [ nearfield_forces(Γs, reflect_xz.(horseshoes), U, Ω, ρ)[end:-1:1]; nearfield_forces(Γs, horseshoes, U, Ω, ρ) ]
...
"""
function nearfield_dynamics(Γs, horseshoes :: AbstractVector{<: Horseshoe}, freestream :: Freestream, r_ref, ρ :: Real, symmetry :: Bool)
