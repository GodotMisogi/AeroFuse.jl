...
plot_CD = begin
    Plots.plot(label = :none, ylabel = L"C_{D_i}")
...
#%%
plot_CY = begin
    Plots.plot(label = :none, ylabel = L"C_Y")
...
plot_CL = begin
    Plots.plot(label = :none, ylabel = L"C_L")
...
p = Plots.plot(plot_CD, plot_CY, plot_CL, size = (800, 700), layout = (3, 1))

savefig(p, "plots/911.pdf")
...
## CDi
plt_CDi_ff =
    Plots.plot(camera = (75, 30), ylabel = L"\alpha", xlabel = "V", zlabel = L"C_{D_i}")
[
    Plots.plot!(res_p[:, 1, n], res_p[:, 2, n], res_p[:, 3, n], label = "", c = :black) for
    n in axes(res_p, 3)
]
...
[
    Plots.plot!(
        res_p[:, 1, n], res_p[:, 2, n], res_p[:, 8, n], label = "", c = :black) for
    n in axes(res_p, 3)
]

# Cm
plt_Cm_ff = Plots.plot(
    camera = (100, 30),
    ylabel = L"\alpha",
    xlabel = "V",
    zlabel = L"C_m",
...
[
    Plots.plot!(
        res_p[:, 1, n], res_p[:, 2, n], res_p[:, 10, n], label = "", c = :black) for
    n in axes(res_p, 3)
]
